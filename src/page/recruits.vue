<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="发布人" width="150">
        <template slot-scope="{row}">
          <span>{{ getTeacherName(row.adminId) }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="发布人联系方式" width="150">
        <template slot-scope="{row}">
          <span>{{ row.phone }}</span>
        </template>
      </el-table-column>
...
<script>
// 接口数据
import { fetchRecruitsList, createRecruits, updateRecruits, deleteRecruits } from '@/api/recruits'
