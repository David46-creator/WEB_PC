<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <h2><span class="link-type">{{ getTeacherName(this.$route.params.id) }}</span> 的巡查记录</h2>
      ...
      <!--      <el-select v-model="listQuery.companyName" class="filter-item" placeholder="巡查企业名称" filterable>-->
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" class-name="small-padding fixed-width" label="操作" width="230">
        <template slot-scope="{row,$index}">
          ...
          <!--          <el-tooltip content="查看巡查记录" placement="top" effect="dark">-->
          <!--            <el-button type="warning" icon="el-icon-document-remove" circle />-->
          <!--          </el-tooltip>-->
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { fetchpatrolAdminByIdList } from '@/api/patrolAdmin'
...
export default {
  ...
  methods: {
    // 接口函数获取数据
    getList() {
      fetchpatrolAdminByIdList(this.$route.params.id).then(response => {
